...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if removes:
            ...
            # of command executions.
            result = self._execute_module(
                module_name='stat', module_args=dict(path=removes),
                task_vars=task_vars, tmp=tmp, persist_files=True)
            stat = result.get('stat', None)
        ...
        sudoable = True
        # set file permissions, more permissive when the copy
        # is done as a different user
        if (self._connection_info.become and
                self._connection_info.become_user != 'root'):
            chmod_mode = 'a+rx'
            sudoable = False
        else:
            chmod_mode = '+rx'
        ...
        result = self._low_level_execute_command(
            cmd=script_cmd, tmp=None, sudoable=sudoable)
